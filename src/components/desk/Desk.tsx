  ...
  return (
    <div className="stage">
      <div className="office">
        ...
        {/* Printed papers feed out of the slot's mouth (fixed % of the 3:2 frame) */}
        <div className="print-mouth">
          <PapersLayer />
        </div>

        {/* Pager device — press for clerk */}
        <button
          className={`pager-btn${paged ? " is-active" : ""}`}
          onClick={page}
          aria-label="Press the pager to call the clerk"
        >
          <img src="/assets/pager.png" alt="" aria-hidden />
          {unread > 0 ? <span className="pager-lamp">{unread}</span> : null}
        </button>

        {!paged && desk.docs.length === 0 ? (
          <div className="hint">Press the pager to call the clerk</div>
        ) : null}

        {/* Clerk dialogue */}
        {paged ? (
          <div className="dialogue" role="dialog" aria-label="Clerk">
            <img className="dialogue__portrait" src="/assets/clerk.png" alt="" aria-hidden />
                  ...
                  return (
                    <li key={i}>
                      <button
                        ...
                        onClick={() =>
                          o.action.type === "service" ? choose(o.action.service) : setPaged(false)
                        }
                      >
...
function PapersLayer() {
  ...
  return (
    <>
      {docs.map((d) => (
        <DeskDocument key={d.id} doc={d} docked />
      ))}
    </>
  );
}
